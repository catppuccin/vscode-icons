...
export type ColorName =
  | AccentName
  | Extract<MonochromaticName, 'text' | 'overlay1'>
...
export const palettes = {
  ...flavorEntries.reduce(
    (acc, [flavorName, flavor]) => ({
      ...
      [flavorName]: flavor.colorEntries
        .filter(
          ([color, { accent }]) =>
            accent || ['text', 'overlay1'].includes(color),
        )
  ...
  'css-variables': flavors.latte.colorEntries
    .filter(
      ([color, { accent }]) => accent || ['text', 'overlay1'].includes(color),
    )
    .map(([color]) => [color, `var(--vscode-ctp-${color})`]) as Array<
    [ColorName, string]
  >,
}
